...
按业务字段判定 视频类/传感类/控制告警类 并对应处理入口）、火情帧
时延观察（2.2.5 观察点生产只读版：最近 N 条 fire 短信 业务时间戳→
服务器接收 的实测时延，短信时间戳 HH:MM 分钟级为粗估 ~；`RADIO_FIRE_COUNT`
调条数，默认 5）、中转服务器上的 STAT 对账 ssh 指引（只看计数，不看
payload）。
