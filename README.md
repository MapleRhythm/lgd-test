...
过滤门并清空全部 jsonl 台账），同时把模型上行链路/流程标志一并回基线
（上行由 2.2.3 步骤6 重新拉起；卫星低速率承载队列一并清空，旧帧不跨
会话上星），并向本地中转补发幂等 `/recover` 清掉
上一会话 `link_block --stop` 留下的服务器组屏蔽——该屏蔽在常驻的本地
中转进程里不随终端退出复位，不清的话 5G 面板会一直显示 DOWN 而接收表
各记各的（仅本地中转且演示云端指向本机时补发，远端中转为生产只读）；
`run_2_2_3.sh` / `run_2_2_4.sh` / `run_2_2_5.sh`
...
| 10 端到端链路数据查询 | `./query_link_data.sh`（5G 信道/短波工控设备/卫星接入模块三张接收表，表窗 10s/2min/10min + 端到端核对；卫星约 2 分钟承载一条、服务器压约 2 分钟后到核心，行滞后约 2 分钟出现） | 云端 |
...
| 目录 | 部署到 | 关键动作 |
|---|---|---|
| `production/edge/` | 边缘网关真机 | `./run_gateway.sh`（默认连生产中转 47.99.47.169:11500；短波/卫星联调帧默认推专用转发链路入口 11450）；`./edge_forward.sh --start` 建立转发 |
