...
printf '\n'
printf '  业务发送（三路依次输入即可，可同时运行；后台启动——一行 [LAUNCH]（pid/\n'
printf '  日志路径）即回提示符，明细写 .protocol-test/sender-*.log，与生产包一致）:\n'
printf '    ./start_video_stream.sh                  # 只发一条（count=1 兜底）\n'
...
printf '  提前停止：kill [LAUNCH] 行里打印的 pid（发送器按条数/时长自行结束）\n'
printf '\n'
...
printf '\n'
printf '  未给 --count/--duration 时只发一条（与生产包一致）；边缘网关受理前\n'
