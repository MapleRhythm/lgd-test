...
printf '\n'
printf '  业务发送（三路依次输入即可，可同时运行；默认后台启动——一行「已启动」\n'
printf '  即回提示符，progress 明细写 .protocol-test/sender-*.log）:\n'
...
printf '    ./start_xxx.sh --stop      # 停止对应后台发送（只 kill 记录的 pid）\n'
...
printf '\n'
printf '  后台发送默认时长 600s 自行结束（SENDER_DURATION 可覆盖）；边缘网关受理前\n'
